...
def scale(current_height, current_width):
    ...
    try:
        if current_height >= current_width:
            height_scale = current_height / current_width
            width_scale = 1.0
        else:
            width_scale = current_width / current_height
            height_scale = 1.0
...
def get_format(height, width, square_footage):
    ...
    width_feet = round(width/12, 2)

    return f'{height}" | {height_feet}\'\n' \
        + f'{width}" | {width_feet}\'\n' \
        + f'{round(height_feet * width_feet, 2)}\''
